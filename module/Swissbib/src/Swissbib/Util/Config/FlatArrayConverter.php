 ...
 *
 * Utility component that converts flat arrays into multi-dimensional arrays
 * based on key-path delimiters. The delimiter is set to '.' my default which
 * allows you to pass in arrays (or ini-file paths) with key in form of
 * 'my.1.key.path'.
 *
 * @category VuFind
 * @package  Swissbib\Util\Config
...
{
    /**
     * The default value used as delimiter, if none was specified on
     * construction time.
     */
    ...
    {
        $this->_keyPathDelimiter = is_string($keyPathDelimiter)
            ? $keyPathDelimiter : self::DEFAULT_KEY_PATH_DELIMITER;
    }

    /**
     * Converts the given source array into a multi-dimensional array. It
     * assumes an associative array and expects each key to be a path to some
     * property where each path component is separated by the key path
     * delimiter passed in to the constructor. In case the key path of a
     * property contains a numeric component it will be converted into a number
     * and is used as numeric index of the according sub-array.
     *
